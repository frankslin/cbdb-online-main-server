...
| 可新增的代碼表（char-variant-map） | 逐操作不同，見〈代碼表與複合實體寫入〉 | char_variant_map | 僅 direct | ✔ | ✘（403，已停用） |
...
- 代碼表分兩組：**只能改**的一組（`config/code_table_mutations.php`，如 nianhao、office_codes、dynasties、choronym_codes、ganzhi_codes 等）送 `create`／`delete` 會回 501；**可新增**的一組現在只剩 `char_variant_map`（`config/code_table_writes.php`）。代碼表的 `delete` 一律不開放——`char_variant_map` 回 **403**（防止級聯刪除人物資料），其餘代碼表回 501。**`TEXT_CODES` 的裸表 create（resource=text-codes）已於 2026-08 下架**（現回 501），新增／刪除文獻請改走 `text-entity` 聚合資源（見 13.4）。
...
只剩一張表開放新增（**`TEXT_CODES` 的 `text-codes`／`text_codes`／`textcodes` 裸表 create 已於 2026-08 下架**，現回 501；文獻的新增／刪除改走 13.4 的 `text-entity` 聚合，會一併處理拼音派生、書名字形標準化與版本列）：
...
- `TEXT_CODES` 的**修改**（13.1 的 `text_codes`，僅 `c_title` 拼音欄）仍開放，與聚合並存（同 office_codes 拼音欄的先例）。

### 13.3 代碼表刪除：一律不開放

代碼表被大量人物資料以外鍵引用，刪一列可能影響數萬筆記錄且難以乾淨復原，因此刪除**已停用**：

- `char_variant_map` 且 `mode=direct` → **403**（`代碼表刪除已停用（防止級聯刪除人物資料）`）
- 其他代碼表（含已下架的 `text-codes`），或 `mode=proposal` → **501**（找不到對應 handler）
