...
### 2.2 書籍／文本（Text，含 collection／instance）— ✅ 聚合已收斂（2026-08，codes UI 封寫暫緩）
...
- mutation resource＝`text-entity`（`text`／`texts` 是人物子資源 BIOG_TEXT_DATA 的既有別名，
  ...
  聚合根。裸表 create/delete（`text-codes`，code_table_writes）已下架；**codes UI 封寫暫緩**——
  裸表編輯頁仍有實體頁未對齊的功能（作者列表面板、instance 載入動作與部分版本欄位），
  parity 補齊後把兩表加入 `closed_code_tables` 即自動封寫。
...
| 書籍／文本 | TEXT_CODES ＋ TEXT_INSTANCE_DATA | ✅ | ✅ CRUD（text-entity，含版本列對賬、成環護欄）| ✅（/app/text；側欄「文獻代碼表」已改指此頁）| 🟡（API 裸表 create/delete 已下架；codes UI 封寫暫緩待 parity——作者面板、instance 專屬欄位）| ❌（同 office／social，走通用 handler 的聚合意圖提案）|
...
橫向優先級建議：先收斂 **office → social institution**（已有明確存儲過程、批量工具現成），再處理 **text（含 instance，✅ 2026-08 已收斂至 step 3＋API 裸寫下架；codes UI 封寫待 parity）**與 **place（含座標）**這兩個層級更深的實體。
