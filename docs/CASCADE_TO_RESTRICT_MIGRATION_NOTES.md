...
>
> 應用層垫片（同 commit）：`OfficeDeleteHandler` 是本批詞表唯一活硬刪路徑
> （`OfficeImportService::delete()`：先刪 OFFICE_CODE_TYPE_REL、有 POSTED_TO_OFFICE_DATA
> 引用護欄回 409）。翻轉後若仍有漏網引用（如 POSTED_TO_ADDR_DATA 殘留列），DELETE 撞 1451、
> 交易回滾；已捕捉 errno 1451 轉友好 409，其他 QueryException 照舊上拋。`/codes` destroy 與
> `CodeTableDeleteHandler` 先前已無條件封堵；app 內無其他硬刪呼叫點（含 console commands）。
