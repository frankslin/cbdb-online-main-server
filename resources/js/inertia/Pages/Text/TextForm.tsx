...
export default function TextForm({ mode, textId, initial, initialLabels, extantOptions, urls }: Props) {
    ...
    const [instances, setInstances] = useState<InstanceRow[]>(initial?.instances ?? []);

    const [busy, setBusy] = useState(false);
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [serverError, setServerError] = useState<string | null>(null);

    const setInst = (i: number, patch: Partial<InstanceRow>) =>
        setInstances((prev) => prev.map((r, idx) => (idx === i ? { ...r, ...patch } : r)));
    const removeInst = (i: number) => setInstances((prev) => prev.filter((_, idx) => idx !== i));
    const addInst = () =>
        setInstances((prev) => {
                            ...
                            {instances.map((r, i) => (
                                <div key={i} className="rounded-md border border-border p-3">
                                    <div className="mb-2 flex items-center justify-between">
                                        ...
                                        <button type="button" className="text-sm text-red-600 hover:underline" onClick={() => removeInst(i)}>
