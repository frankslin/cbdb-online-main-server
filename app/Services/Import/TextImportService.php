...
class TextImportService implements EntityAggregateService {
    ...
    /**
     * 書名字形標準化（char_variant_map 寬鬆模式，峯→峰）：改寫**落庫書名本身**，
     * 讓 c_title_chn／拼音／無拼音檢查看到同一字形（與批量匯入一致）。
     *
    ...
    protected function textColumns(array $input): array {
        ...
        return [
            'c_title_chn' => $title,
            'c_title' => $pinyin,
            'c_title_trans' => $str($input['title_trans'] ?? null),
            'c_title_alt_chn' => $str($input['title_alt_chn'] ?? null),
    ...
    public function create(array $input, int $actorPersonId = 0): array {
        ...
        $variantReplacements = $this->standardizeTitleVariants($this->normalizeTitle((string) $input['title']))['variant_replacements'];
        ...
        return [
            ...
            'variant_replacements' => $variantReplacements,
    ...
    protected function instancePayload(int $textId, array $r): array {
        return [
            ...
            'c_instance_title' => $r['title_pinyin'] !== null
                ? $r['title_pinyin']
                : ($r['title_chn'] !== null ? $this->buildTitlePinyin((string) $r['title_chn']) : null),
