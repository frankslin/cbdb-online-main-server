...
- **背景**：全站 16 個 React 表單（13 個人物編輯器、Codes 新增／編輯、office／social-institution／text-entity 實體表單）原本並排「直接儲存」「提交建議」兩顆鈕；有直接寫入權者按「提交建議」還要再過一次 `ProposalModeDialog`（取消／提交建議／直接保存三選一）。彈窗的目的只是防止有權限者誤送不必要的提案，但每次提案都多一步。
...
- **收斂**：16 個呼叫點各自的 `confirmProposalMode` state 與 `ProposalModeDialog` 一併移除；`direct_save_prompt_*`（biogmains／codes）與 `proposal_confirm_*`（office／social_institution／text_entity）翻譯鍵刪除，改在 shared `common` 加 `more_submit_options`／`proposal_menu_hint` 兩鍵。Codes 頁順帶對齊人物編輯器：只可提案者只看到「提交提案」（原本也顯示一顆必吃 403 的「直接儲存」）。
